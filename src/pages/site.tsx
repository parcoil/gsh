  ...
  useEffect(() => {
    async function getSite() {
      const { data, error } = await supabase
        .from("sites")
        .select("id, created_at, site_name, site_url, site_image, site_description, votes, discord_url")
        .eq("id", Number(id))
        .single();
    ...
    <div className="flex justify-center py-10 px-4">
      <Card className="max-w-xl w-full space-y-6">
        ...
        <img
            ...
            alt={site.site_name}
            className="rounded-xl mb-6 max-h-40 max-w-full  bg-muted"
            loading="lazy"
          />
          <div className="flex justify-between items-center">
            
            <h1 className="text-3xl font-bold scroll-m-20">{site.site_name}</h1>
            <div className={"flex items-center space-x-2" + (hasVoted ? 'cursor-not-allowed' : 'cursor-pointer')}>
              <Button
                onClick={handleVote}
                disabled={hasVoted || isVoting}
                aria-label="Vote for this site"
              >
                <ArrowUpIcon className={`h-5 w-5 ${hasVoted ? 'fill-current' : ''}`} />
        ...
        <CardContent className="space-y-4">
          ...
          {site.discord_url && (
            <a href={site.discord_url} target="_blank" rel="noopener noreferrer" className="text-primary hover:text-primary/80 underline"><Button>
              <ExternalLink/> Join Discord Server</Button></a>
          )}
